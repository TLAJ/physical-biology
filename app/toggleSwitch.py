        ...
        for k in np.arange(0, 4.1, 0.15):
            ...
            if unit_scale > 0:
                plt.quiver(
                    j,
                    k,
                    x_direction / unit_scale,
                    y_direction / unit_scale,
                    scale=40,
                    width=0.005,
                    headlength=3,
                    headaxislength=3,
...
plot(
    1.0,
    nullcline(1.0, a[0]),
    checkStablePoint(a[0], 1.0, nullcline(1.0, a[0])),
    markersize=10,
)
# 2つ目の固定点解析
figure()
...
plot(
    vertex[0].evalf(),
    nullcline(vertex[0].evalf(), a[1]),
    checkStablePoint(a[1], vertex[0].evalf(), nullcline(vertex[0].evalf(), a[1])),
    markersize=10,
)
plot(
    vertex[2].evalf(),
    nullcline(vertex[2].evalf(), a[1]),
    checkStablePoint(a[1], vertex[2].evalf(), nullcline(vertex[2].evalf(), a[1])),
    markersize=10,
)
plot(
    vertex[3].evalf(),
    nullcline(vertex[3].evalf(), a[1]),
    checkStablePoint(a[1], vertex[3].evalf(), nullcline(vertex[3].evalf(), a[1])),
    markersize=10,
)
"""
